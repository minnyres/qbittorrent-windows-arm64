...
pacman -S --needed diffutils p7zip mingw-w64-${runtime}-boost mingw-w64-${runtime}-clang mingw-w64-${runtime}-cmake mingw-w64-${runtime}-qt6-base mingw-w64-${runtime}-qt6-svg mingw-w64-${runtime}-qt6-tools mingw-w64-${runtime}-qt6-translations
...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_MODULE_PATH="C:/libtorrent/share/cmake/Modules" -DCMAKE_INSTALL_PREFIX="C:/qbittorrent" -DSTACKTRACE=OFF -DQT6=ON ..
cmake --build .
...
mkdir -p qbittorrent
cd qbittorrent
cp ../qbittorrent-${qbittorrent_ver}/build/qbittorrent.exe .
